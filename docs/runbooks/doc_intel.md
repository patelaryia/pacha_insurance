...
Transport exhaustion leaves the current durable stage failed for an explicit safe retry.
...
`pending|failed -> running` transition. A crash deliberately leaves `running`; after
...
The predicate is exactly handwritten OR stored coverage below 0.05; 0.05 is not
eligible. Native coverage combines geometric glyph area with the configured
native-character density measure in the pack.
...
stage state and unique `(claim_id, check_id, input_fingerprint)` consistency key.
